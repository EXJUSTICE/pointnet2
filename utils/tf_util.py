...
def _variable_with_weight_decay(name, shape, stddev, wd, use_xavier=True):
  ...
  """
  if use_xavier:
    initializer = tf.keras.initializers.glorot_uniform()   # Replaced Xavier Initializer with glorot for compatibility
  else:
...
def batch_norm_template(inputs, is_training, scope, moments_dims_unused, bn_decay, data_format='NHWC'):
  ...
  return tf.layers.batch_normalization(inputs,    # Replaced contrib layers with this one for TF2
